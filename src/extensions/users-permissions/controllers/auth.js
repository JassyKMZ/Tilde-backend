...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue', sans-serif; background-color: #f5f5f5; padding: 20px; margin: 0;">
  ...
  async emailConfirmation(ctx) {
    ...
    try {
      ...
      // Re-read from DB so the response reflects the persisted state.
      // Depending on Strapi internals, the object returned by confirmEmail
      // can be stale while the DB row is already updated.
      ...
      const isConfirmed = Boolean(freshUser?.confirmed ?? user.confirmed);
      const email = freshUser?.email || user.email || null;
